 ...
 * @author Decebal Suiu
 */
public class InsertAfterColumnAction extends AbstractAction {

    private int selectedColumn = -1;

    public InsertAfterColumnAction() {
        this(-1);
    }

    public InsertAfterColumnAction(int selectedColumn) {
        super();
        this.selectedColumn = selectedColumn;
        putValue(Action.NAME, I18NSupport.getString("insert.column.after.action.name"));
    }

    public void actionPerformed(ActionEvent event) {
        final NumberSelectionPanel panel = new NumberSelectionPanel(I18NSupport.getString("insert.column.number"));
        final BaseDialog dialog = new BaseDialog(panel, I18NSupport.getString("insert.column.after.action.name"), true) {
        ...
        for (int i = 0; i < columnCount; i++) {
            reportGridPanel.insertColumns(cell.getRow(), cell.getColumn() + i, 1, true);
        }
        ...
        Globals.getReportUndoManager().addEdit(new LayoutEdit(oldLayout, newLayout, I18NSupport.getString("edit.column.insert.after")));
    }
